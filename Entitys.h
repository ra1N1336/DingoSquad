...
public:
	...
	/*float GetPen() {//getPenetrationPower
		return *(float*)((DWORD)this + (DWORD)0x864);
	}
	int GetDamage() {//getDamage
		return *(int*)((DWORD)this + (DWORD)0x868);
	}
	float GetRange() {//getRange
		return *(float*)((DWORD)this + (DWORD)0x86C);
	}
	float GetRangeModifier() {//getRangeModifier
		return *(float*)((DWORD)this + (DWORD)0x870);
	}
	float GetArmorRatio() {//weaponArmorRatio
		return *(float*)((DWORD)this + (DWORD)0x850);
	}
	float GetStandInacc() {//inaccuracyStand
		return *(float*)((DWORD)this + (DWORD)0xA30);
	}
	float GetJumpInacc() {//inaccuracyJump
		return *(float*)((DWORD)this + (DWORD)0xA48);*/
	//}
};
...
public:
	...
	int m_bSpotted() {
		static int VecOrigin = GET_NETVAR("DT_BaseEntity", "m_bSpotted");
		return *(int*)((DWORD)this + VecOrigin);
	}
	bool* IsSpottedPtr()
	{
		static int iOffset = GET_NETVAR("DT_BaseEntity", "m_bSpotted");
	...
	bool BombDefused()
	{
		static int m_bBombDefused = GET_NETVAR("DT_PlantedC4", "m_bBombDefused");
	...
	bool isAlive() {
		BYTE lifestate = *(BYTE*)((DWORD)this + 0x025B);//m_lifeState
		return (lifestate == 0);
	}
	int GetFlags() {
	...
	int GetTeam() {
		return *reinterpret_cast<int*>((DWORD)this + 0xF0);//_iTeamNum
	}
	...
	float GetSimulationTime(){
		return *(float*)((DWORD)this + (DWORD)0xC4560E44);
		}
	float GetAnimTime() {
		return *(float*)((DWORD)this + (DWORD)0xD27E8416);
	}
	int GetiShots() {
	...
	bool HasKit()
	{
		static int iOffset = GET_NETVAR("DT_CSPlayer", "m_bHasDefuser");
	...
	float* GetFlashMaxAlpha() {
		static int GetFlashMaxAlpha = GET_NETVAR("DT_CSPlayer", "m_flFlashMaxAlpha");
		return GetFieldPointer<float>(GetFlashMaxAlpha);
	}
...
public:
	...
	int* ModelIndex() {
		return (int*)((DWORD)this + (DWORD)0x254);
	}
	int* ViewModelIndex() {
		return (int*)((DWORD)this + (DWORD)0x29B0);
	}
	int* WorldModelIndex() {
		return (int*)((DWORD)this + (DWORD)0x31E4);
	}
	...
	bool IsKnife() {
		if (GetWepType(this->GetItemDefinitionIndex()) == WEAPON_KNIFE) {
			return true;
		}
		return false;
	}
	...
	/*CCSWeaponInfo* GetCSWpnData() {
		static DWORD GetCSWpnDataAddr = Utils.PatternSearch("client.dll", (PBYTE)"\x55\x8B\xEC\x81\xEC\x00\x00\x00\x00\x53\x57\x8B\xF9\xBB\x00\x00\x00\x00", "xxxxx????xxxxx????", NULL, NULL);//updated
		if (GetCSWpnDataAddr) {
